...
bool my_ping (char destination_ip[100]) {
	...
	dst.sin_addr.s_addr = inet_addr(destination_ip);
  	//inet_pton (AF_INET, destination_ip, &(dst.sin_addr));
...
*/
		...
		printf("\n\n\n########################################################################\n");
		printf("\nSending ICMP Echo Request to : %s", inet_ntoa(dst.sin_addr));
	...
	if (icmphd2->type == 11 && icmphd2->code == 0) {	//type 11 code 0 is Time to Live exceeded in transit message
		...
		printf("\nReply from hop no. %d Address: %s Time to live Exceeded\n\n",hop, inet_ntoa (hop_addr.sin_addr));
      	...
      	{
      	printf ("\nReached destination: %s with hop limit: %d",inet_ntoa (hop_addr.sin_addr), hop);	//else assumed type 0 will be recieved
									//normally type 0 has been reieved in this code other than 11 & 8

	if ( ! strcmp(inet_ntoa (hop_addr.sin_addr), destination_ip)) { 
		//printf("\nDestination reached... \n");
		return ping_ok;	
	}
